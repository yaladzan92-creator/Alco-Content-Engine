...
export function validateBlueprint(blueprint: StrategyBlueprint): {
  ...
  if (!blueprint.brand_identity?.brand_name?.trim()) {
    missing.push('Brand Name (brand_identity.brand_name)');
  }
  if (!blueprint.target_audience?.primary_audience?.trim()) {
    missing.push('Primary Audience (target_audience.primary_audience)');
  }
  if (
    !blueprint.target_audience?.audience_problem ||
    blueprint.target_audience.audience_problem.length === 0
  ) {
    missing.push('Audience Pain Points (target_audience.audience_problem)');
  }
  if (
    !blueprint.positioning?.core_positioning?.trim() &&
    (!blueprint.positioning?.usp || blueprint.positioning.usp.length === 0)
  ) {
    missing.push('Core Positioning or USP (positioning.core_positioning)');
  }
  if (!blueprint.offer?.main_offer?.trim()) {
    missing.push('Main Offer (offer.main_offer)');
  }
  if (!blueprint.messaging?.core_message?.trim()) {
...
export function mapCampaignPackToBlueprint(cp: any): IntakeParseResult {
  ...
  if (!category) category = 'Software / Product Campaign';
  ...
  if (!brandSummary && campaignName) {
    brandSummary = `Meta Ads Campaign Pack: ${campaignName}`;
  }
  ...
  if (!primaryAudience) {
    ...
    primaryAudience = parts.length > 0 ? parts.join(' | ') : 'Digital Marketer & Online Sellers';
  }
  ...
  if (audienceProblem.length === 0) {
    audienceProblem = ['Kesulitan mengeksekusi campaign beriklan', 'Takut boncos karena tidak punya struktur iklan terarah'];
  }
  ...
  if (!objections || objections.length === 0) {
    if (Array.isArray(cp.assumptions) && cp.assumptions.length > 0) {
      ...
      objections = ['Apakah cara ini cocok untuk pemula?', 'Apakah hasilnya terbukti instan?'];
    }
  }
  ...
  if (!corePositioning && cp.creative_strategy?.primary_angle) {
    corePositioning = `Sistem campaign ${cp.creative_strategy.primary_angle} berbasis visual hook dan ad copy terstruktur.`;
  }
  if (!corePositioning) {
    corePositioning = `Meta Ads Campaign Pack: ${campaignName || 'Meta Ads Strategy'}`;
  }
  ...
  // 4. OFFER
  let mainOffer = cp.offer?.main_offer || '';
  if (!mainOffer) {
    if (Array.isArray(cp.copy_assets?.ctas) && cp.copy_assets.ctas[0]) {
      const text = typeof cp.copy_assets.ctas[0] === 'string' ? cp.copy_assets.ctas[0] : cp.copy_assets.ctas[0]?.text;
      if (text) mainOffer = `Akses ${brandName} (${text})`;
    } else if (Array.isArray(cp.image_ads) && cp.image_ads[0]?.cta) {
      mainOffer = `Akses ${brandName} - ${cp.image_ads[0].cta}`;
    } else {
      mainOffer = `Akses Penawaran Utama ${brandName}`;
    }
  }

  let offerType = cp.offer?.offer_type || 'Digital Product / Campaign Pass';
  ...
  if (!coreMessage) {
    coreMessage = `Meningkatkan efisiensi & hasil penjualan untuk ${brandName}.`;
  }
  ...
  if (!brandVoice) brandVoice = 'Direct, Practical, Urgent & Conversion-Focused';
  ...
  if (!copyDirection || copyDirection.length === 0) {
    ...
    if (Array.isArray(cp.copy_assets?.ctas) && cp.copy_assets.ctas.length > 0) {
      const text = typeof cp.copy_assets.ctas[0] === 'string' ? cp.copy_assets.ctas[0] : cp.copy_assets.ctas[0]?.text;
  ...
  if (!contentPillars || contentPillars.length === 0) {
    contentPillars = [
      'TOFU: Problem Awareness & Visual Hooks',
      'MOFU: Creative Strategy Demo & Value Proposition',
      'BOFU: Direct Offer, Ad Assets & Conversion CTA'
    ];
  }

  const mappedBlueprint: StrategyBlueprint = {
    project_id: cp.project_id || `campaign_pack_${Date.now()}`,
    project_name: campaignName || brandName || 'Meta Ads Campaign Pack',
    brand_identity: {
    ...
    content_strategy: {
      ...
      campaign_theme: campaignName || 'Meta Ads Conversion Campaign',
      channel_notes: ['Meta Ads (Instagram & Facebook)'],
    },
  };
  ...
  return {
    ...
    strategy_context: {
      ...
      content_pillars: blueprint.content_strategy?.content_pillars || [
        ...
        'Offer Clarification & Objection Handling',
