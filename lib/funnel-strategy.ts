...
export function buildFunnelStrategyFromContext(
  ...
  if (options?.userOverrides && (options.userOverrides.tofu !== undefined || options.userOverrides.mofu !== undefined || options.userOverrides.bofu !== undefined)) {
    const rawTofu = options.userOverrides.tofu ?? 0;
    const rawMofu = options.userOverrides.mofu ?? 0;
    const rawBofu = options.userOverrides.bofu ?? 0;
    distribution = {
      tofu: Math.max(0, rawTofu),
      mofu: Math.max(0, rawMofu),
      bofu: Math.max(0, rawBofu),
      total_posts: rawTofu + rawMofu + rawBofu,
